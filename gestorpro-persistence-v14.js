/* GestorPro — Persistência V24
   Supabase é a fonte oficial dos dados do painel.
   O estado window.D é apenas o estado de renderização da interface.
   As funções originais de cadastro/edição continuam responsáveis pela regra de negócio;
   esta camada apenas persiste no Supabase depois que elas realmente concluírem a operação.
*/
(function(){
  ...
  async function persist(){if(saveInFlight)return saveInFlight;saveInFlight=(async()=>{const a=await api(),session=await a.session();if(!session)throw new Error('Sessão do usuário não encontrada.');const c=await a.client(),d=realD();if(!d||!Array.isArray(d.servers)||!Array.isArray(d.clients))throw new Error('Estado do GestorPro ainda não está pronto.');const r=await c.rpc('gestorpro_save_data',{p_data:JSON.parse(JSON.stringify(d))});if(r.error)throw r.error;return true})();try{return await saveInFlight}finally{saveInFlight=null}}
  ...
  async function runOriginalAndPersist(original,event,successMessage,operationName){
    const previousSave=window.save;
    let originalReachedSave=false;
    window.save=function(){originalReachedSave=true;render();return true};
    try{
      await original.call(this,event);
      if(!originalReachedSave)return false;
      await persist();
      toast(successMessage);
      return true;
    }catch(err){
      console.error('[GestorPro] '+operationName,err);
      toast('Erro ao salvar: '+(err.message||err));
      return false;
    }finally{window.save=previousSave}
  }
  function install(){
    ...
    window.saveServer=async function(e){e?.preventDefault?.();return runOriginalAndPersist.call(this,originalServer,e,'Servidor salvo no banco de dados','saveServer')};
    window.saveClient=async function(e){e?.preventDefault?.();return runOriginalAndPersist.call(this,originalClient,e,'Cliente salvo no banco de dados','saveClient')};
    ...
    window.gpEditServer=editServer;window.gpDeleteServer=deleteServer;
    console.log('[GestorPro] V24 persistência oficial instalada');return true;
  }
  async function hydrate(){
    ...
    try{
      ...
      assignD(incoming);hydrated=true;install();render();
      console.log('[GestorPro] dados carregados exclusivamente do Supabase');return true;
